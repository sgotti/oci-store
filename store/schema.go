...
var dbCreateStmts = [...]string{
	...
	// blobinfo table. The primary key is "digest".
	"CREATE TABLE IF NOT EXISTS blobinfo (digest string, mediatype string);",
	"CREATE UNIQUE INDEX IF NOT EXISTS digestidx ON blobinfo (digest);",

	// data table. The primary key is "key".
	"CREATE TABLE IF NOT EXISTS data (key string, value blob, datatype string, digest string);",
	"CREATE UNIQUE INDEX IF NOT EXISTS keyidx ON data (key);",
}
