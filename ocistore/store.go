...
func (s *OCIStore) ReadBlob(digest string) (io.ReadCloser, error) {
	...
	defer gcLock.Close()
	return s.Store.ReadBlob(digest)
}
...
func (s *OCIStore) WriteBlob(r io.ReadSeeker, mediaType string) (string, error) {
	...
	defer gcLock.Close()
	return s.Store.WriteBlob(r, mediaType)
}
...
func (s *OCIStore) ListBlobs(mediaType string) ([]string, error) {
	...
	defer gcLock.Close()
	return s.Store.ListBlobs(mediaType)
}
...
func (s *OCIStore) DeleteBlob(digest string) error {
	...
	defer gcLock.Close()
	return s.Store.DeleteBlob(digest)
}
...
func (s *OCIStore) GetRef(name string) (*common.Descriptor, error) {
	...
	defer gcLock.Close()
	data, err := s.Store.GetData(name, MediaTypeDescriptor)
	if err != nil {
...
func (s *OCIStore) SetRef(name string, d *common.Descriptor) error {
	...
	return s.Store.SetData(name, data, MediaTypeDescriptor, "")
}
...
func (s *OCIStore) DeleteRef(name string) error {
	...
	defer gcLock.Close()
	return s.Store.DeleteData(name, MediaTypeDescriptor)
}
...
func (s *OCIStore) ListAllRefs() ([]string, error) {
	...
	defer gcLock.Close()
	return s.Store.ListAllDataKeys(MediaTypeDescriptor)
}
...
func (s *OCIStore) checkAllDataAvailable(name string) (bool, error) {
	...
	minfo, err := s.Store.GetBlobInfo(pathDigest(d.Digest))
	if err != nil {
		return false, err
	}
	if minfo == nil {
	...
	mblob, err := s.Store.ReadBlob(d.Digest)
	if err != nil {
		return false, err
	}
	defer mblob.Close()
	...
	cinfo, err := s.Store.GetBlobInfo(pathDigest(m.Config.Digest))
	if err != nil {
		return false, err
	}
	if cinfo == nil {
	...
	for _, l := range m.Layers {
		linfo, err := s.Store.GetBlobInfo(pathDigest(l.Digest))
		if err != nil {
			return false, err
		}
		if linfo == nil {
...
func (s *OCIStore) GC() error {
	...
	// Get all manifests
	mdigests, err := s.Store.ListBlobs(MediaTypeManifest)
	if err != nil {
		return err
	}

	// Get all refs
	refs, err := s.Store.ListAllDataKeys(MediaTypeDescriptor)
	if err != nil {
		return err
	}
	...
	for _, r := range refs {
		data, err := s.Store.GetData(r, MediaTypeDescriptor)
		if err != nil {
	...
	cdigests, err := s.Store.ListBlobs(MediaTypeImageSerializationConfig)
	if err != nil {
		return err
	}
	ldigests, err := s.Store.ListBlobs(MediaTypeImageSerialization)
	if err != nil {
		return err
	}
	...
	refsmap = map[string]struct{}{}
	for _, mdigest := range mdigests {
		mr, err := s.Store.ReadBlob(mdigest)
		if err != nil {
			return err
		}
		defer mr.Close()
	...
	for _, olddigest := range olddigests {
		// Ignore delete errors
		s.Store.DeleteBlob(olddigest)
	}

	// Call base store GC
	if err := s.Store.GC(); err != nil {
		return err
	}
